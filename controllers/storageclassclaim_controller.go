...
func (r *StorageClassClaimReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.storageClassClaim.Spec.ConsumerID == "" {
		if len(storageClientList.Items) > 1 {
			r.log.Info("Multiple StorageClient resources found, but no ConsumerID specified.")
			return reconcile.Result{}, fmt.Errorf("multiple StorageClient resources found, but no ConsumerID specified")
		}
		r.storageClient = &storageClientList.Items[0]
	} else {
		for _, storageClient := range storageClientList.Items {
			if storageClient.Status.ConsumerID == r.storageClassClaim.Spec.ConsumerID {
	...
	if r.storageClient == nil {
		return reconcile.Result{}, fmt.Errorf("no external storage consumer id found on the " +
			"StorageClient status, cannot determine mode")
	}
