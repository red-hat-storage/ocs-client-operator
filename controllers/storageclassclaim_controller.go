...
func (r *StorageClassClaimReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.list(ocsClientList, client.InNamespace(r.OperatorNamespace)); err != nil {
		return reconcile.Result{}, err
	}

	switch l := len(ocsClientList.Items); {
	case l == 0:
		return reconcile.Result{}, fmt.Errorf("no OcsClient found")
	case l != 1:
		return reconcile.Result{}, fmt.Errorf("multiple OcsClient found")
	}
	r.ocsClient = &ocsClientList.Items[0]

	var result reconcile.Result
	var reconcileError error

	// StorageCluster checks for required fields.
	switch ocsClient := r.ocsClient; {
	case ocsClient.Status.ConsumerID == "":
		return reconcile.Result{}, fmt.Errorf("no external storage consumer id found on the " +
			"OcsClient status, cannot determine mode")
	case ocsClient.Spec.StorageProviderEndpoint == "":
		return reconcile.Result{}, fmt.Errorf("no external storage provider endpoint found on the " +
			"OcsClient spec, cannot determine mode")
	}
...
func (r *StorageClassClaimReconciler) reconcileConsumerPhases() (reconcile.Result, error) {
	...
	providerClient, err := providerclient.NewProviderClient(
		r.ctx,
		r.ocsClient.Spec.StorageProviderEndpoint,
		10*time.Second,
	)
	if err != nil {
		return reconcile.Result{}, err
...
func (r *StorageClassClaimReconciler) getCephFSStorageClass(data map[string]string) *storagev1.StorageClass {
	...
	storageClass := &storagev1.StorageClass{
		...
		Provisioner:          fmt.Sprintf("%s.cephfs.csi.ceph.com", r.ocsClient.Namespace),
		Parameters:           data,
	}
	return storageClass
}
...
func (r *StorageClassClaimReconciler) getCephRBDStorageClass(data map[string]string) *storagev1.StorageClass {
	...
	storageClass := &storagev1.StorageClass{
		...
		ReclaimPolicy:        &pvReclaimPolicy,
		AllowVolumeExpansion: &allowVolumeExpansion,
		Provisioner:          fmt.Sprintf("%s.rbd.csi.ceph.com", r.ocsClient.Namespace),
		Parameters:           data,
	}
	return storageClass
}
