...
func (s *StorageClientReconciler) onboardConsumer(instance *v1alpha1.StorageClient, externalClusterClient *providerClient.OCSProviderClient) (reconcile.Result, error) {
	...
	// TODO Need to find a way to this one as its OCP dependent
	...
	// TODO the name of stoageClient starts with `StorageConsumer-` in ODF
	name := fmt.Sprintf("StorageClient-%s-%s", instance.Name, clusterVersion.Spec.ClusterID)
...
func (s *StorageClientReconciler) createDefaultStorageClassClaimsForRBD(instance *v1alpha1.StorageClient) error {
	storageClassClaimBlock := &v1alpha1.StorageClassClaim{
		...
		Spec: v1alpha1.StorageClassClaimSpec{
			Type: "blockpool",
			StorageClient: &v1alpha1.StorageClientNamespacedName{
				Name:      instance.Name,
				Namespace: instance.Namespace,
			},
		},
