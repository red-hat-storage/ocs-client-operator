...
func (r *StorageClientReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetCache().IndexField(ctx, &v1alpha1.StorageClient{}, storageProviderEndpointIndexName, func(o client.Object) []string {
		res := []string{}
		...
		return fmt.Errorf("unable to set up FieldIndexer for storageclient providerendpoint: %v", err)
	}
	...
	if err := mgr.GetCache().IndexField(ctx, &v1alpha1.StorageClaim{}, ownerIndexName, func(obj client.Object) []string {
		...
		return fmt.Errorf("unable to set up FieldIndexer for storageclaim owner: %v", err)
	}
...
func (r *StorageClientReconciler) reconcilePhases() (ctrl.Result, error) {
	storageClientListOption := []client.ListOption{
		// TODO: this isn't fool proof, need to revisit as a storageclient can be created to same provider but via different endpoint incase of nodeport
		client.MatchingFields{storageProviderEndpointIndexName: r.storageClient.Spec.StorageProviderEndpoint},
	}

	storageClientList := &v1alpha1.StorageClientList{}
	if err := r.Client.List(r.ctx, storageClientList, storageClientListOption...); err != nil {
		r.Log.Error(err, "unable to list storage clients")
		return ctrl.Result{}, err
	}

	if len(storageClientList.Items) > 1 {
		r.Log.Info("one StorageClient is allowed with a unique storageproviderendpoint but found more than one. Rejecting new request.")
		r.storageClient.Status.Phase = v1alpha1.StorageClientFailed
	...
	if controllerutil.AddFinalizer(r.storageClient, storageClientFinalizer) {
		...
		if err := r.update(r.storageClient); err != nil {
			r.Log.Info("Failed to update StorageClient with finalizer.", "StorageClient", r.storageClient.Name)
	...
	if err := r.reconcileStorageClaims(); err != nil {
		return reconcile.Result{}, err
	}
...
func (r *StorageClientReconciler) verifyNoStorageClaimsExist() error {
	...
	err := r.Client.List(r.ctx,
		storageClaims,
		client.MatchingFields{ownerIndexName: r.storageClient.Name},
		client.Limit(1),
	)
	if err != nil {
...
func (r *StorageClientReconciler) reconcileStorageClaims() error {

	if err := r.createBlockStorageClaim(); err != nil && !kerrors.IsAlreadyExists(err) {
		return fmt.Errorf("failed to create block storageclaim: %v", err)
	}
	if err := r.createSharedfileStorageClaim(); err != nil && !kerrors.IsAlreadyExists(err) {
...
func (r *StorageClientReconciler) createBlockStorageClaim() error {
	blockClaim := &v1alpha1.StorageClaim{}
	blockClaim.Name = fmt.Sprintf("%s-ceph-rbd", r.storageClient.Name)
	blockClaim.Spec.Type = "block"
	blockClaim.Spec.StorageClientName = ptr.To(r.storageClient.Name)
	if err := r.own(blockClaim); err != nil {
		return fmt.Errorf("failed to own storageclaim of type block: %v", err)
	}
	return r.create(blockClaim)
}

func (r *StorageClientReconciler) createSharedfileStorageClaim() error {
