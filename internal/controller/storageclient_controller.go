...
func (r *storageClientReconcile) reconcileResource(obj client.Object, rawObject []byte) error {
	...
	if utils.IsForbiddenError(err) {
		key := client.ObjectKeyFromObject(obj)
		if err := r.Client.Get(r.ctx, key, obj); client.IgnoreNotFound(err) != nil {
			return fmt.Errorf("failed to replace %v %v: %v", obj.GetObjectKind().GroupVersionKind(), obj.GetName(), err)
		}
		if err := mutateFunc(); err != nil {
			return fmt.Errorf("failed to replace %v %v: %v", obj.GetObjectKind().GroupVersionKind(), obj.GetName(), err)
		}
		if newKey := client.ObjectKeyFromObject(obj); key != newKey {
			return fmt.Errorf("failed to replace %v %v: MutateFn cannot mutate object name and/or object namespace", obj.GetObjectKind().GroupVersionKind(), obj.GetName())
		}
