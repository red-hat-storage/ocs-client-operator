	...
	enqueueStorageClientRequest := handler.EnqueueRequestsFromMapFunc(
		func(ctx context.Context, _ client.Object) []reconcile.Request {
			...
			for idx := range storageClients.Items {
				request = append(request, reconcile.Request{NamespacedName: client.ObjectKeyFromObject(&storageClients.Items[idx])})
			}
			return request
		},
	)
