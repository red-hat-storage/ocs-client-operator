...
func TestReconcileOperatorServiceMonitor_CANotYetInjected(t *testing.T) {
	caCM := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "openshift-service-ca.crt", Namespace: testNamespace},
	}
	r := newSMSReconciler(t, caCM)
	err := r.reconcileOperatorServiceMonitor()
	assert.NoError(t, err)

	sm := &monitoringv1.ServiceMonitor{}
	err = r.Get(r.ctx, types.NamespacedName{Name: templates.OperatorServiceMonitorName, Namespace: testNamespace}, sm)
	assert.True(t, kerrors.IsNotFound(err), "ServiceMonitor should not be created when CA not yet injected")
}

func TestReconcileOperatorServiceMonitor(t *testing.T) {
	caCM := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "openshift-service-ca.crt", Namespace: testNamespace},
		Data:       map[string]string{"service-ca.crt": "fake-ca-cert"},
	}
