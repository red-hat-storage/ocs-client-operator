...
func (c *OperatorConfigMapReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	s3EndpointsConfigMapPredicates := builder.WithPredicates(
		predicate.NewPredicateFuncs(
			func(obj client.Object) bool {
				...
				labelValue, hasS3EndpointsLabel := labels[s3EndpointsConfigMapLabelKey]
				return hasS3EndpointsLabel && labelValue == strconv.FormatBool(true)
			},
		),
	)
	...
	bldr := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&corev1.ConfigMap{},
			enqueueConfigMapRequest,
			s3EndpointsConfigMapPredicates,
		).
		Watches(
			&corev1.Secret{},
			enqueueConfigMapRequest,
...
func (c *OperatorConfigMapReconciler) buildDesiredNginxDataWithProxies() (map[string]string, error) {
	...
	if c.operatorConfigMap.Data != nil {
		if disableS3EndpointProxy, _ := strconv.ParseBool(c.operatorConfigMap.Data[disableS3EndpointProxyKey]); disableS3EndpointProxy {
...
func (c *OperatorConfigMapReconciler) computeDesiredProxyConfigByKey(out map[string]string) error {
	...
	for i := range extList.Items {
		...
		configKey := nginxProxyConfigKeyPrefix + uniqueIdentifier + nginxProxyConfigKeySuffix
