...
//+kubebuilder:rbac:groups=noobaa.io,resources=noobaas,verbs=get;delete
...
func (c *OperatorConfigMapReconciler) removeNoobaaOperator() error {
	...
	if client.IgnoreNotFound(err) != nil {
		return err
	} else if noobaaSubscription.UID == "" {
		return nil
	}
	index := slices.IndexFunc(
	...
	if csv.Name != "" {
		if err := c.get(csv); client.IgnoreNotFound(err) != nil {
			...
			if err := c.delete(csv); client.IgnoreNotFound(err) != nil {
