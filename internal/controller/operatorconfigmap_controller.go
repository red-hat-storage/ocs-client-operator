...
func (c *OperatorConfigMapReconciler) reconcileODFSnapshotterSubscription() error {
	odfSnapshotterSubscription, err := c.getSubscriptionByPackageName("odf-csi-snapshotter")
	if err != nil {
		return err
	}
	if c.subscriptionChannel != "" && c.subscriptionChannel != odfSnapshotterSubscription.Spec.Channel {
		...
		if err := c.update(odfSnapshotterSubscription); err != nil {
			return fmt.Errorf("failed to update subscription channel of 'odf-csi-snapshotter' to %v: %v", c.subscriptionChannel, err)
		}
	}
	return nil
}
