...
const (
	...
	csiImagesConfigMapLabel    = "ocs.openshift.io/images.version"
)
...
func (c *OperatorConfigMapReconciler) reconcileDelegatedCSI() error {
	...
	} else if rbdCsiDriver.UID != "" && utils.AddAnnotation(rbdCsiDriver, "ceph.csi.io/ownerref", string(bytes)) {
	...
	} else if cephFsCsiDriver.UID != "" && utils.AddAnnotation(cephFsCsiDriver, "ceph.csi.io/ownerref", string(bytes)) {
