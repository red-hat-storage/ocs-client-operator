...
func (c *OperatorConfigMapReconciler) hasSnapshotsWithNfsDriver() (bool, error) {
	vscList, err := c.listNfsDriverSnapshots()
	if err != nil {
		return false, fmt.Errorf("failed to list NFS driver snapshots: %v", err)
	}
	return len(vscList.Items) > 0, nil
}

// listNfsDriverPVs lists all PersistentVolumes with NFS driver
func (c *OperatorConfigMapReconciler) listNfsDriverPVs() (*corev1.PersistentVolumeList, error) {
	pvList := &corev1.PersistentVolumeList{}
	if err := c.list(pvList, client.MatchingFields{pvDriverIndexName: templates.NfsDriverName}); err != nil {
		return nil, fmt.Errorf("failed to list NFS driver PVs: %v", err)
	}
	return pvList, nil
}

// listNfsDriverSnapshots lists all VolumeSnapshotContent with NFS driver
func (c *OperatorConfigMapReconciler) listNfsDriverSnapshots() (*snapapi.VolumeSnapshotContentList, error) {
	vscList := &snapapi.VolumeSnapshotContentList{}
	if err := c.list(vscList, client.MatchingFields{vscDriverIndexName: templates.NfsDriverName}); err != nil {
		return nil, fmt.Errorf("failed to list NFS driver snapshots: %v", err)
	}
	return vscList, nil
}
