...
func (s *SubscriptionAdmission) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	desiredUserSubcriptionChannel := subscription.Spec.Channel
	for idx := range storageClients.Items {
		...
		if desiredClientChannel, exist := storageClient.GetAnnotations()[utils.DesiredSubscriptionChannelAnnotationKey]; !exist {
			s.Log.Info(fmt.Sprintf("Rejecting review as storageclient %q doesn't have desired subscription channel annotation", namespacedName))
			return admission.Errored(http.StatusInternalServerError, fmt.Errorf("storageclient %q doesn't has required subscription channel annotation", namespacedName))
		} else if desiredClientChannel != desiredUserSubcriptionChannel {
			...
			return admission.Denied(fmt.Sprintf("subscription channel %q not allowed as it'll violate storageclient %q requirements", desiredUserSubcriptionChannel, namespacedName))
		}
	}
