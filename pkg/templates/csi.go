...
func SetSecurityContextConstraintsDesiredState(scc *secv1.SecurityContextConstraints, ns string) {
	...
	scc.Users = []string{
		fmt.Sprintf("system:serviceaccount:%s:cephfs-provisioner-sa", ns),
		fmt.Sprintf("system:serviceaccount:%s:cephfs-plugin-sa", ns),
		fmt.Sprintf("system:serviceaccount:%s:rbd-provisioner-sa", ns),
		fmt.Sprintf("system:serviceaccount:%s:rbd-plugin-sa", ns),
	}
}
...
var CSIOperatorConfigSpec = csiopv1a1.OperatorConfigSpec{
	DriverSpecDefaults: &csiopv1a1.DriverSpec{
		...
		Plugin: &csiopv1a1.PluginSpec{
			KubeletDirPath:  "/var/lib/kublet",
...
var CephFSDriverSpec = csiopv1a1.DriverSpec{
	Provisioner: &csiopv1a1.ProvisionerSpec{
		PodCommonSpec: csiopv1a1.PodCommonSpec{
			Labels: map[string]string{
