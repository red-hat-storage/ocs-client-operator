...
func (c *ResourceCollector) collectOdfVGSCCount(ctx context.Context, ch chan<- prometheus.Metric) {
	vgscList := &odfgsapiv1b1.VolumeGroupSnapshotContentList{}
	if err := c.client.List(ctx, vgscList); err != nil {
		return
	}

	count := 0
	for i := range vgscList.Items {
		if vgscList.Items[i].Spec.Driver == templates.CephFsDriverName {
			count++
		}
	}

	desc := prometheus.NewDesc(
		"ocs_client_operator_cephfs_volume_group_snapshot_content_count",
		"Number of CephFS VolumeGroupSnapshotContent resources on this client cluster",
		nil, nil,
	)
	m, err := prometheus.NewConstMetric(desc, prometheus.GaugeValue, float64(count))
	if err != nil {
