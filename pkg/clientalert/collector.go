...
var _ prometheus.Collector = &ClientAlertCollector{}

// ClientAlertCollector exposes alerts fetched from the storage provider as
...
type ClientAlertCollector struct {
	clientAlert *ClientAlert
}

// NewClientAlertCollector creates a collector that reads from ca's in-memory alert cache.
func NewClientAlertCollector(ca *ClientAlert) *ClientAlertCollector {
	return &ClientAlertCollector{clientAlert: ca}
}

// Describe implements prometheus.Collector.
// Empty on purpose -- signals an unchecked collector to the registry because
// label names vary per alert.
func (c *ClientAlertCollector) Describe(chan<- *prometheus.Desc) {}

// Collect implements prometheus.Collector.
func (c *ClientAlertCollector) Collect(ch chan<- prometheus.Metric) {
	allAlerts := c.clientAlert.GetAllAlerts()
